...
pub fn execute_ssh_status() -> Result<()> {
    ...
    for (idx, host) in app_config.ssh_hosts.iter().enumerate() {
        ...
        eprint!("   Status: ");
        match smol::block_on(async { async_compat::Compat::new(ssh::test_connection(host, None)).await })
        {
            Ok(result) => {
                if result.success {
                    dure_info!(" Connected");
...
pub fn execute_ssh_add(
    ...
    match smol::block_on(async { async_compat::Compat::new(ssh::test_connection(&ssh_host, None)).await })
    {
        Ok(result) => {
            if result.success {
