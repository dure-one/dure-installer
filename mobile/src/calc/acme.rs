...
fn delete_cloudflare_record(
    ...
    log::debug!("Deleting Cloudflare record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name (from config): {}", record_name);
    log::debug!("  Type: {}", record_type);
    log::debug!(
        "  API Token: {}...",
    ...
    // Find zone ID
    log::debug!("  Calling Cloudflare API to find zone...");
    let zone = match client.find_zone_by_domain(domain) {
        Ok(Some(z)) => {
            log::debug!("  ✓ Found zone: {} ({})", z.name, z.id);
            z
        }
        Ok(None) => {
            log::debug!("  ❌ Zone not found for domain: {}", domain);
            anyhow::bail!("Zone not found for domain: {}", domain)
        }
        Err(e) => {
            log::debug!("  ❌ Error finding zone: {}", e);
            return Err(e);
        }
    };
    ...
    log::debug!("  Searching with full name: '{}'", full_name);

    log::debug!(
        "  Searching for record with name='{}' type='{}'",
        full_name, record_type
    );
    log::debug!("  Calling Cloudflare API to find record...");
    ...
    match client.find_record(&zone.id, &full_name, record_type) {
        Ok(Some(record)) => {
            ...
            log::debug!("    Name: {}", record.name);
            log::debug!("    Type: {}", record.record_type);
            log::debug!("    Content: {}", record.content);
            log::debug!("  Calling Cloudflare API to delete record...");
            ...
            match client.delete_record(&zone.id, &record.id) {
                ...
                Err(e) => {
                    log::debug!("  ❌ Error deleting record: {}", e);
                    Err(e)
                }
            }
        }
        Ok(None) => {
            ...
            log::debug!("  Getting all records for zone to debug...");

            match client.get_records(&zone.id) {
                Ok(records) => {
                    log::debug!("  All records in zone:");
                    for rec in &records {
                        log::debug!(
                            "    - {} ({}) -> {}",
                ...
                Err(e) => {
                    log::debug!("  ❌ Error listing records: {}", e);
                }
            }
        ...
        Err(e) => {
            log::debug!("  ❌ Error finding record: {}", e);
            Err(e)
        }
    }
}
...
fn set_cloudflare_a_record(
    ...
    log::debug!("Setting Cloudflare A record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name: {}", record_name);
    log::debug!("  IP: {}", ip);
    log::debug!(
        "  API Token: {}...",
        &api_token.chars().take(8).collect::<String>()
    ...
    log::debug!("  Found zone: {} ({})", zone.name, zone.id);
    ...
    match client.find_record(&zone.id, &record_name, "A")? {
        Some(existing) => {
            log::debug!("  Updating existing record (ID: {})", existing.id);
            client.update_record(&zone.id, &existing.id, &record_name, "A", ip, None, None)?;
            log::debug!("  ✓ Updated A record");
        }
        None => {
...
fn set_cloudflare_txt_record(
    ...
    log::debug!("Setting Cloudflare TXT record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name: {}", record_name);
    log::debug!("  TXT Value: {}", txt_value);
    log::debug!(
        "  API Token: {}...",
        &api_token.chars().take(8).collect::<String>()
    );
...
fn set_cloudflare_aaaa_record(
    ...
    log::debug!("Setting Cloudflare AAAA record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name: {}", record_name);
    log::debug!("  IPv6: {}", ipv6);
    log::debug!(
...
fn set_duckdns_a_record(token: &str, domain: &str, ip: &str) -> Result<()> {
    ...
    log::debug!("Setting DuckDNS A record: {} -> {}", domain, ip);
    log::debug!("Token: {}...", &token.chars().take(8).collect::<String>());
...
fn set_duckdns_txt_record(token: &str, domain: &str, txt_value: &str) -> Result<()> {
    // Reference: https://github.com/acmesh-official/acme.sh/blob/master/dnsapi/dns_duckdns.sh
    log::debug!("Setting DuckDNS TXT record: {} -> {}", domain, txt_value);
    log::debug!("Token: {}...", &token.chars().take(8).collect::<String>());
...
fn set_duckdns_aaaa_record(token: &str, domain: &str, ipv6: &str) -> Result<()> {
    ...
    log::debug!("Setting DuckDNS AAAA record: {} -> {}", domain, ipv6);
    log::debug!("Token: {}...", &token.chars().take(8).collect::<String>());
...
fn set_porkbun_a_record(api_token: &str, domain: &str, subdomain: &str, ip: &str) -> Result<()> {
    ...
    log::debug!("Setting Porkbun A record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Subdomain (input): {}", subdomain);
    log::debug!("  IP: {}", ip);
    ...
    log::debug!("  Subdomain (for API): '{}'", porkbun_subdomain);

    let client = PorkbunClient::new(api_key.to_string(), secret_key.to_string());

    // For Porkbun: domain in URL, subdomain in body
    log::debug!("  Creating record via Porkbun API...");
    log::debug!("    URL: /dns/create/{}", domain);
    log::debug!(
...
fn set_porkbun_txt_record(
    ...
    // Reference: https://github.com/acmesh-official/acme.sh/blob/master/dnsapi/dns_porkbun.sh
    log::debug!("Setting Porkbun TXT record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Subdomain (input): {}", subdomain);
    log::debug!("  TXT Value: {}", txt_value);
...
fn set_porkbun_aaaa_record(
    ...
    // Reference: https://github.com/acmesh-official/acme.sh/blob/master/dnsapi/dns_porkbun.sh
    log::debug!("Setting Porkbun AAAA record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Subdomain (input): {}", subdomain);
    log::debug!("  IPv6: {}", ipv6);
...
fn delete_porkbun_record(
    ...
    log::debug!("Deleting Porkbun record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name: {}", record_name);
    log::debug!("  Type: {}", record_type);
    ...
    log::debug!("  Calling Porkbun delete API...");
    log::debug!(
...
fn set_gcp_a_record(api_token: &str, domain: &str, record_name: &str, ip: &str) -> Result<()> {
    log::debug!("Setting GCP A record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name: {}", record_name);
    log::debug!("  IP: {}", ip);
    ...
    if let Some(_existing) = existing {
        log::debug!("  Updating existing A record");
        client.update_rrset(
            project_id,
            &zone.name,
            &fqdn,
        ...
        log::debug!("  Creating new A record");
        client.create_rrset(
            project_id,
            &zone.name,
            &fqdn,
...
fn set_gcp_aaaa_record(api_token: &str, domain: &str, record_name: &str, ipv6: &str) -> Result<()> {
    log::debug!("Setting GCP AAAA record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name: {}", record_name);
    log::debug!("  IPv6: {}", ipv6);
    ...
    if let Some(_existing) = existing {
        log::debug!("  Updating existing AAAA record");
        client.update_rrset(
            project_id,
            &zone.name,
            &fqdn,
        ...
        log::debug!("  Creating new AAAA record");
        client.create_rrset(
            project_id,
            &zone.name,
            &fqdn,
...
fn set_gcp_txt_record(
    ...
    log::debug!("Setting GCP TXT record");
    log::debug!("  Domain: {}", domain);
    log::debug!("  Record name: {}", record_name);
    log::debug!("  TXT Value: {}", txt_value);
    ...
    if let Some(_existing) = existing {
        log::debug!("  Updating existing TXT record");
        client.update_rrset(project_id, &zone.name, &fqdn, "TXT", 300, vec![quoted_txt])?;
        log::debug!("  ✓ Updated TXT record");
    } else {
        log::debug!("  Creating new TXT record");
